...
$(document).on('click', '.shop__item', function () {
    
    let prodId = $(this).attr('id');
    $('#sendProduct_id').val(prodId);

    let prodPrice = Number($(this).children('.product__price').text()).toFixed(2);
    $('.custom-form__price').text(prodPrice);

    let delivePrice = Number(0).toFixed(2);

    $('input[type="radio"]').on('click', function () {

        let isDelivery = $("#dev-yes").prop("checked");

        if (isDelivery && prodPrice < 2000) {
          delivePrice = Number(280).toFixed(2);
          $('.custom-form__dprice').text(delivePrice);
          let summ = +delivePrice + +prodPrice;
          $('.custom-form__summ').text(summ);
        } else {
          delivePrice = Number(0).toFixed(2);
          $('.custom-form__dprice').text(delivePrice);
          let summ = +delivePrice + +prodPrice;
          $('.custom-form__summ').text(summ);
        }
...
if (shopList) {
  ...
  shopList.addEventListener('click', (evt) => {
    ...
    if (prod.some(pathItem => pathItem.classList && pathItem.classList.contains('shop__item'))) {
      ...
      const buttonOrder = shopOrder.querySelector('#send_order');
      ...
      buttonOrder.addEventListener('click', (evt) => {
        ...
        if (inputs.every(inp => !!inp.value)) {
          ...
          const buttonEnd = popupEnd.querySelector('#proceed');

          buttonEnd.addEventListener('click', () => {

            $("#order_form").load(location.href + " #order_form"); // очистка формы
...
if (document.querySelector('.shop-page')) {
  ...
  var min_price = Number($('#min_price1').val());
  var max_price = Number($('#max_price1').val());
  var user_min_price = Number($('#min_price').val());
  var user_max_price = Number($('#max_price').val());

  $('.range__line').slider({
    min: min_price,
    max: max_price,
    values: [user_min_price, user_max_price],
    range: true,
